...
app = Client("DX_COIN_V4", api_id=API_ID, api_hash=API_HASH, bot_token=BOT_TOKEN, in_memory=True)
...
async def add_sudo(client, message: Message):
    if message.from_user.id not in INIT_SUDO: return
    if not message.reply_to_message: return await message.reply(f"<b>{B} ➲ ʀᴇᴘʟʏ ᴛᴏ ᴀ ᴜsᴇʀ!</b>")
    user = message.reply_to_message.from_user
    cursor.execute("UPDATE users SET is_sudo = 1 WHERE user_id = ?", (user.id,))
    db.commit()
    await message.reply_text(f"<b>{B} ➲ {get_mention(user.id, user.first_name)} ɪs ɴᴏᴡ sᴜᴅᴏ! ✨</b>")
...
async def add_coin(client, message: Message):
    ...
    if len(parts) < 2: return await message.reply(f"<b>{B} ➲ ᴀᴍᴏᴜɴᴛ ᴅᴀᴡ!</b>")
    try:
        amount = int(parts[1])
        ...
        await message.reply_text(
            ...
            f"<b>➲ sᴛᴀᴛᴜs :</b> <code>{old_val} ➜ {new_val}</code>\n"
